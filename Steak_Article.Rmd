...
tb <- sdat %>% # get data subset
  select(age) %>% # subset age 
  table() %>% # put in table
  prop.table()*100 # calculate proportion and multiply by 100 to get percentage
...
knitr::kable(dt, format="html",
             col.names=c("Ages","%"),
             digits=2, # After the decimal place
             caption="Ages of Survey Respondents") %>%
  kableExtra::kable_styling(bootstrap_options ="striped", # styling
                            full_width=FALSE)
```
